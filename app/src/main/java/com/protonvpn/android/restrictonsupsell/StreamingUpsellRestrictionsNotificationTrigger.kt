...
class ShowStreamingUpsellNotification @Inject constructor(
    ...
    operator fun invoke() {
        ...
        val launchIntent =
            createLaunchIntent.forNotification().apply {
...
@Singleton
class StreamingUpsellRestrictionsNotificationTrigger
@Inject
constructor(
    ...
    private suspend fun markNotificationShown() {
        restrictionsUpsellStore.update { current ->
            current.copy(streaming = current.streaming.copy(lastNotificationTimestamp = now()))
        }
    }
}
