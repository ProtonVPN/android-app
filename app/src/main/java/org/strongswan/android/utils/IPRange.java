...
public class IPRange implements Comparable<IPRange> {
    ...
    public List<IPRange> remove(IPRange range) {
         ...
         * | range |     | range   |    |   range | */
            if (compareAddr(mFrom, range.mFrom) < 0 && compareAddr(range.mTo, mTo)
                < 0) {    /* the removed range is completely within our boundaries:
             * |    this    |
             *   | range |   */
        ...
        if (mPrefix != null) {
            ...
            for (i = from.length - 1; i >= commonByte; i--) {
                ...
                for (bit = 7; bit >= bitMin; bit--) {
                    ...
                    fromCur = from[i] & mask;
                    if (fromPrev == 0
                        && fromCur != 0) {    /* 0 -> 1: subnet is the whole current (right) subtree */
                        list.add(new IPRange(from.clone(), netmask));
                        fromFull = false;
                    }
                    else if (fromPrev != 0
                        && fromCur == 0) {    /* 1 -> 0: invert bit to switch to right subtree and add it */
                        from[i] ^= mask;
                        list.add(new IPRange(from.clone(), netmask));
                        fromCur = 1;
                    }
                    ...
                    toCur = to[i] & mask;
                    if (toPrev != 0
                        && toCur == 0) {    /* 1 -> 0: subnet is the whole current (left) subtree */
                        list.add(new IPRange(to.clone(), netmask));
                        toFull = false;
                    }
                    else if (toPrev == 0
                        && toCur != 0) {    /* 0 -> 1: invert bit to switch to left subtree and add it */
                        to[i] ^= mask;
                        list.add(new IPRange(to.clone(), netmask));
                        toCur = 0;
                    }
            ...
            if (fromFull && toFull) {    /* full subnet (from=to or from=0.. and to=1.. after common
            prefix) - not
                reachable
             * due to the shortcut at the top */
                list.add(new IPRange(from.clone(), prefix));
            }
            else if (fromFull) {    /* full from subnet (from=0.. after prefix) */
                list.add(new IPRange(from.clone(), prefix + 1));
            }
            else if (toFull) {    /* full to subnet (to=1.. after prefix) */
